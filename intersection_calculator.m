...
%The script calculates the real discontinuity intersection using the 3D
%information of discontinuity planes. See Menegoni et al. (2021) for
%more infromation.
%The code incorporates a function of the geom3D lybrary of David Legland.
%References:
%- Menegoni, N., Giordan, D., & Perotti, C. (2021). An Open-Source Algorithm for 3D ROck Slope Kinematic Analysis (ROKA). Applied Sciences, 11(4), 1698. MDPI AG. Retrieved from https://doi.org/10.3390%2Fapp11041698
%- David Legland (2021). geom3d (https://www.mathworks.com/matlabcentral/fileexchange/24484-geom3d), MATLAB Central File Exchange. Retrieved August 30, 2021. 
...
if ispc
    ...
    writetable(Tintersections,fullfile([pathname],tablefilenameTXT));
    tablefilenameXLSX = (['Intersection_calculated.xlsx']);
    writetable(Tintersections,fullfile([pathname],tablefilenameXLSX));
elseif ismac
    tablefilenameTXT = (['Intersection_calculated.txt']);
    writetable(Tintersections,fullfile([pathname],tablefilenameTXT));
    tablefilenameXLSX = (['Intersection_calculated.xlsx']);
